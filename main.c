...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    while(getz(s,BUF_SIZE) != EOF) {
       line++;
       sp=skipblanks(s);
       if ((n=sscanf(sp,"%lf %lf",&x,&y))==2) {
	  savepoint(x,y);
       } else if (*sp == '#') {		// comment line
	    ;
       } else if (strncmp(sp,"nextygraph",10)==0) {
	   nextygraph();
	   ...
	   exit(0);
       } else if (strncmp(sp,"noback",6)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"back",4)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"jump",4)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"label",5)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"symbol",6)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"tagsize",7)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"scalesize",9)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"symbolsize",10)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"autopen",7)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"noautosymbol",12)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"pen",3)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"noautopen",9)==0) {
	   savecmd(sp);
       } else if (strncmp(sp,"line",4)==0) {
	   savecmd(sp);
       } else {
	  if (*sp != '\0' && *sp != '\n') {
